"""
gradaing_helper.py
  This code is specifically designed to make grading lab courses, Physics 275 for now, easier
  at Physics & Astronomy Department, Rutgers University.
  The code gathers the notebook files at one folder, and after the person does put the comments on the notebook files,
  it calculates the grades, and then updates the Sakai folder system.
  This code helps to use the Download All and Upload All functionality of Sakai much more efficiently.
Usage:
  python grading_helper.py [--mode {nb, gr, up}] [--path main_path]
  --path argument is where the bulk_download folder sits. Default is '.'.
  --mode argument is what you want to do,
  nb: Gathers all the nb files inside the bulk_download folder and puts them inside nb_files folder.
...
class session:
  ...
  def find_reports(self):
    ...
    self.reports_list = []
    path = self.bulk_download_path_string
    path_nb = self.nb_files_path_string

    try:
      os.chdir(path_nb)
      print('>> Finding reports inside the nb_files folder. Not copying anything.')
      print('-> If your nb_files folder is empty, delete the folder and run "grading_helper --mode nb" again.')
      print('-> Be sure to have the bulk_download folder in the main path, i.e. main_path/bulk_download.\n')
      result = []
      ...
      print('Found reports in the lab folder and copied them to nb_files folder. This makes grading a little easier.')
      result = []
      ...
      os.chdir(self.bulk_path_string)
      os.mkdir('nb_files')
      os.chdir(path_nb)

      for report_path in result:
        shutil.copy2(report_path, path_nb)
        new_path = os.path.join(path_nb, os.path.basename(report_path))
  ...
  def update_comments_txt_sakai(self):
    ...
    for lr in lab_report_list:
        ...
        name = name.split('_');
        match_found = 0
        # Will run through the names in lab path student folders and find the
        # matching names (first and last names) in graded folder.
        ...
        # This might be caused by nicknames or aliases.
        if not match_found:
          print('>> There is a name mismatch when trying to copy the comments.txt for,')
          print(lr)
          print('-> Will try only the last name. Usually this is a nickname issue.')
          print('-> When grading be sure to check the "Names:" line.')
        ...
        if not match_found:
          print('!! Problem occurred when processing these names and trying to update the comments.txt file: ')
          print('-! ' + name)
          print('-! Two probable causes:\n   a. the students not putting their names on the "Names: " line of the report, or\n   b. the students have not submitted the lab yet.\n')

    if len(f_filled) != len(folder_list):
      print('!! The comment were not placed for the following folders:')
      print('!- ', set(folder_list)-set(f_filled))
    if len(lr_placed) != len(lab_report_list):
      print('!! The following comment were not placed in folders:')
  ...
  def update_grades_csv_sakai(self):
    ...
    for line in blkf:
      ...
      if (flag == 0):
        ...
        if not match_found:
          # If there is a grades.csv file and we do not have graded item for it,
          # probably no submission.
          #print('-> ' + 'First Name: ' + line_list[f_name_col] + ', Last Name: ' + line_list[l_name_col])
          no_submission_names.append( (line_list[f_name_col] + line_list[l_name_col]).strip('"') )
          csvf.write(line)

    if no_submission_names:
      print('>> Seems like for these names on grades.csv file, no submission is done,')
      print(no_submission_names)
      print('-> The grade cell on grades.csv for these names is left empty.')
      print('-> This can also happen if someone forgot to put their names on the report.\n')
